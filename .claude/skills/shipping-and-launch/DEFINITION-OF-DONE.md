...
A standing, project-wide bar that every change must clear before it counts as done. Unlike acceptance criteria, which vary per task and answer "did we build the right thing?", the Definition of Done is the same every time and answers "is this finished to our standard?". Use it as the final gate before merge or deploy, alongside the `shipping-and-launch` skill.
...
The depth behind these items lives in this repository's `reviewing-code` skill (structured review with prioritized feedback) and in simplification passes that reduce complexity without changing behavior.
...
- [ ] Backward compatibility considered for any public interface or API change (see `api-and-interface-design`)

### Documentation
- [ ] Public interfaces, APIs, and user-facing behavior are documented
- [ ] Architectural decisions worth preserving are recorded (an ADR or equivalent)
